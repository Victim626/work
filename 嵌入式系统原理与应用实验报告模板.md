# 嵌入式系统原理与应用实验报告

专业班级：计2307班                                 
组员：许洋帆（235150732）姚志昊（235150734）

实验日期：2025年10月21日

## 实验一：裸机开发环境与GPIO实验


#### 一、实验目的

1. 	初步学会使用STM32CubeMX配置外设工作模式和创建裸机工程；
2. 初步学会使用Keil IDE环境便携代码和调试工程；
3. 掌握使用HAL库GPIO。
4. 修改代码，使三个灯轮流闪烁。



#### 二、实验内容

		1.	使用STM32CubeMX配置生成裸机工程
		2.	kei环境使用



#### 三、实验过程

1. 电路图及说明
   <img width="1578" height="498" alt="image" src="https://github.com/user-attachments/assets/a73f2d0f-4c7a-4fd0-850c-13bee598029d" />


　　说明：板子上的绿灯、蓝灯和红灯分别连接到的芯片上的PB0、PB14、PB7引脚，均为高电平点亮
　　2. 基本原理
　　通过代码使得开发版上PB0、PB14、PB7引脚的电平通过一定频率变化，使得蓝绿红三种灯可以轮流闪烁



#### 四、编程

1. 程序流程图

```mermaid
flowchart TD
    A([开始]) --> B[初始化 HAL & GPIO]
    B --> C[点亮 LED1，熄灭 LED2 LED3]
    C --> D[延时 1000 ms]
    D --> E[点亮 LED2，熄灭 LED1 LED3]
    E --> F[延时 1000 ms]
    F --> G[点亮 LED3，熄灭 LED1 LED2]
    G --> H[延时 1000 ms]
    H --> I{是否继续}
    I -->|是| C
    I -->|否| J([结束])
```


2. 实验过程寄存器变化截图及解释（仅实验一）

![4eee40c0bd19830a4a1f663a3c9b505d](https://github.com/user-attachments/assets/298788b5-32ff-4128-8683-af48ba799a96)

截图时 GPIOB 的 MODER 已将 PB5/PB6/PB7 配置为通用输出，ODR 对应位为 0，故三灯均灭；
单步运行 HAL_GPIO_WritePin() 后，ODR 相应位翻转，即可观察到寄存器位变化与 LED 亮灭严格对应。

3. 主要代码段及解释

  
 初始化代码解释在注释里
 
   ```c
static void MX_GPIO_Init(void)
{
  GPIO_InitTypeDef GPIO_InitStruct = {0};        // 定义结构体，存放引脚配置

  /* ① 打开 GPIOB 与 GPIOA 时钟，外设才能工作 */
  __HAL_RCC_GPIOB_CLK_ENABLE();
  __HAL_RCC_GPIOA_CLK_ENABLE();

  /* ② 先把 3 个 LED 引脚拉低 → 共阳 LED 全灭 */
  HAL_GPIO_WritePin(GPIOB, LD0_Pin|LD1_Pin|LD2_Pin, GPIO_PIN_RESET);

  /* ③ 配置 PBx 为推挽输出，无上下拉，低速 */
  GPIO_InitStruct.Pin   = LD0_Pin|LD1_Pin|LD2_Pin; // 同时配置 3 个引脚
  GPIO_InitStruct.Mode  = GPIO_MODE_OUTPUT_PP;     // 推挽输出
  GPIO_InitStruct.Pull  = GPIO_NOPULL;             // 不使能内部上下拉
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;     // 低速已够，功耗低
  HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);          // 写入寄存器，配置生效
}
  ```

   功能实现部分代码解释在注释当中：

   ```c


 while (1)
{
  /* USER CODE BEGIN 3 */
  /* ---- LD0 独闪 ---- */
  HAL_GPIO_WritePin(GPIOB, LD0_Pin, GPIO_PIN_SET);   // LD0 亮（共阳：SET=灭，RESET=亮）
  HAL_Delay(1000);                                   // 保持 1 s
  HAL_GPIO_WritePin(GPIOB, LD0_Pin, GPIO_PIN_RESET); // LD0 灭
  HAL_Delay(1000);                                   // 暗 1 s

  /* ---- LD1 独闪 ---- */
  HAL_GPIO_WritePin(GPIOB, LD1_Pin, GPIO_PIN_SET);   // LD1 亮
  HAL_Delay(1000);
  HAL_GPIO_WritePin(GPIOB, LD1_Pin, GPIO_PIN_RESET); // LD1 灭
  HAL_Delay(1000);

  /* ---- LD2 独闪 ---- */
  HAL_GPIO_WritePin(GPIOB, LD2_Pin, GPIO_PIN_SET);   // LD2 亮
  HAL_Delay(1000);
  HAL_GPIO_WritePin(GPIOB, LD2_Pin, GPIO_PIN_RESET); // LD2 灭
  HAL_Delay(1000);
  /* USER CODE END 3 */
}


   ```




#### 五、实验小结

成功配置 GPIOB 口 LD0/LD1/LD2 三个引脚为推挽输出模式。
在 while(1) 用户代码区实现**“轮流亮灭”**逻辑：
LD0 → 灭 → LD1 → 灭 → LD2 → 灭 → 循环；每个状态持续 1000 ms，现象清晰。
使用 HAL_GPIO_WritePin 单灯控制，一条语句对应一个灯，便于后续扩展与调试。



